...
import pytest
# local imports
from manim_meshes.helpers import are_edges_equal, is_vararray_equal
...
def test_remove_parts():
    ...
    assert m3 == m4
    assert are_edges_equal(m3.edges, m4.edges)
...
def test_remove_faces():
    ...
    assert len(m2.faces) == 3
    assert len(m2.parts) == 2
    assert not m2.dangling_face_check()
    assert are_edges_equal(m2.edges, [(0, 1), (1, 2), (0, 2), (2, 3), (1, 3), (3, 4), (2, 4)])
    # removing one non-dangling face removes the part that uses it and the respective edges
    m3 = deepcopy(m)
    m3.remove_faces([2])
    assert len(m3.vertices) == 4
    assert len(m3.faces) == 3
    assert len(m3.parts) == 1
    assert m3.dangling_face_check()
    assert are_edges_equal(m3.edges, [(0, 1), (1, 2), (0, 2), (2, 3), (1, 3), (3, 4), (1, 4)])
    # removing all faces works and order does not matter
    m4 = deepcopy(m)
    m4.remove_faces([0, 3, 1, 2])
    assert len(m4.vertices) == 4
    assert len(m4.faces) == 0
    assert len(m4.parts) == 0
    assert are_edges_equal(m4.edges, [])
    # removing one face may result in multiple part deletions
    m5 = deepcopy(m)
    m5.remove_faces([1])
    assert len(m5.vertices) == 4
    assert len(m5.faces) == 3
    assert len(m5.parts) == 0
    assert are_edges_equal(m5.edges, [(0, 1), (1, 2), (0, 2), (2, 3), (1, 3), (3, 4), (2, 4), (1, 4)])
...
def test_remove_vertices():
    ...
    assert len(m5.vertices) == 5
    assert len(m5.faces) == 3
    assert len(m5.parts) == 1
    assert are_edges_equal(m5.edges, [(0, 1), (1, 2), (0, 2), (2, 3), (0, 3)])
...
def test_add_faces():
    ...
    assert len(m3.faces) == 1
    assert are_edges_equal(m3.edges, [(0, 1), (1, 2), (0, 2)])
    # add single to existing - and size can be different
    m3.add_faces([np.array([1, 2, 3, 3])])
    assert len(m3.faces) == 2
    assert are_edges_equal(m3.edges, [(0, 1), (1, 2), (0, 2), (2, 3), (3, 3), (1, 3)])
    # add multiple to non-existing
    m4 = deepcopy(m)
    m4.add_faces([np.array([0, 1, 2]), np.array([1, 2, 3])])
    assert len(m4.faces) == 2
    assert are_edges_equal(m4.edges, [(0, 1), (1, 2), (0, 2), (1, 3), (2, 3)])
    # add multiple to existing
    m4.add_faces([np.array([1, 1, 1]), np.array([2, 2, 2, 2])])
    assert len(m4.faces) == 4
    assert are_edges_equal(m4.edges, [(0, 1), (1, 2), (0, 2), (1, 3), (2, 3), (2, 2), (1, 1)])
...
def test_add_vertices():
    ...
    assert len(m3.vertices) == 2
    assert are_edges_equal(m3.edges, m.edges)
    # add multiple
    m4 = deepcopy(m)
    m4.add_vertices(np.array([[0, 0, 0], [1, 1, 1], [1, 2, 3]]))
    assert len(m4.vertices) == 4
    assert are_edges_equal(m4.edges, m.edges)
...
def test_update_face():
    ...
    assert len(m4_1.faces) == 4
    assert m4_1.faces[1][0] == 5
    assert are_edges_equal(m4_1.edges, [(0, 1), (1, 2), (0, 2), (2, 3), (1, 3), (3, 4), (1, 4), (2, 5), (0, 5)])
    m4_2 = deepcopy(m)  # tuple of int
    m4_2.update_face(1, (5, 2, 0))
    assert not m4_2.dangling_vert_check()
    assert len(m4_2.faces) == 4
    assert m4_2.faces[1][0] == 5
    assert are_edges_equal(m4_1.edges, [(0, 1), (1, 2), (0, 2), (2, 3), (1, 3), (3, 4), (1, 4), (2, 5), (0, 5)])
    m4_3 = deepcopy(m)  # 1D - np.ndarray
    m4_3.update_face(1, np.array([5, 2, 0]))
    assert not m4_3.dangling_vert_check()
    assert len(m4_3.faces) == 4
    assert m4_3.faces[1][0] == 5
    assert are_edges_equal(m4_1.edges, [(0, 1), (1, 2), (0, 2), (2, 3), (1, 3), (3, 4), (1, 4), (2, 5), (0, 5)])
...
def test_update_vertex():
    ...
    assert len(m4_3d.vertices) == 6
    assert m4_3d.vertices[0][1] == 1
    m4_2d = deepcopy(m_2d)
    m4_2d.update_vertex(0, np.array([0, 1]))
    assert len(m4_2d.vertices) == 3
    assert np.sum(np.abs(m4_2d.vertices[0] - m4_2d.vertices[1])) == 0
...
def test__add_and_remove_faces():
    ...
    assert len(triangle_mesh.faces) == 4
    assert len(triangle_mesh.parts) == 0
    # add faces
    triangle_mesh.add_faces([np.array([0, 1, 2, 3])])
    assert len(triangle_mesh.faces) == 5
    assert len(triangle_mesh.faces[len(triangle_mesh.faces) - 1]) == 4
    assert len(triangle_mesh.parts) == 0
    # re-connect faces as parts
    triangle_mesh.add_parts([np.array([0, 1, 2, 3, 4])])
    assert len(triangle_mesh.parts) == 1
    # changed triangle only to quad pyramid mesh
    assert triangle_mesh == quad_mesh
    assert are_edges_equal(triangle_mesh.edges, quad_mesh.edges)
...
def test_add_to_mesh():
    ...
    m6 += m6_1
    assert m6_1 == m
    assert len(m6.vertices) == 12
    assert len(m6.faces) == 8
    assert len(m6.parts) == 4
    assert m6.faces[len(m6.faces) - 1][2] == 10
    assert are_edges_equal(m6.edges,
    ...
    m7.add_to_mesh(m7_1)
    assert m7.faces[len(m7.faces) - 1][2] == 0
    assert m7.faces[len(m7.faces) - 1][1] == len(m.vertices) - 1
    assert m7.parts[-1][0] == 0
    assert m7.parts[-1][2] == len(m.faces) - 1
    assert are_edges_equal(m7.edges, m.edges + [(6, 6), (6, 7), (7, 8), (6, 8), (5, 6), (0, 5), (0, 6)])
...
def test_scale_mesh():
    ...
    m1 = deepcopy(m)
    m1.scale_mesh(2)
    elems, count = np.unique(m1.vertices, return_counts=True)
    assert len(elems) == 1
    assert 2 in elems
...
def test_translate_vertex():
    ...
    assert np.array_equal(m1.vertices[1], np.array([3, -1, 5]))
    assert np.array_equal(m1.vertices[0], np.zeros(3))
    assert np.array_equal(m1.vertices[2:], np.zeros((8, 3)))
...
def test_remove_duplicate_vertices():
    m = create_pyramid() + create_pyramid() + create_pyramid()
    m.remove_duplicate_vertices()
    assert len(m.find_vertex(np.array([1, 1, 0]))) == 1
    assert np.array_equal(m.vertices, create_pyramid().vertices)
    assert len(m.faces) == 3 * 6
    assert all(np.array_equal(a, b) for a, b in
               zip(m.faces, create_pyramid().faces + create_pyramid().faces + create_pyramid().faces))
    assert are_edges_equal(m.edges, create_pyramid().edges)
    m2 = Mesh(
        vertices=[[0, 0], [4, 4], [1, 1], [0, 0], [1, 1], [2, 2], [2, 2], [3, 3], [2, 2]],
        faces=[[0, 1, 2], [1, 2, 3], [3, 4, 5], [4, 5, 6], [6, 7, 8]],
    )
    m2.remove_duplicate_vertices()
    assert len(m2.find_vertex(np.array([1, 1]))) == 1
    assert np.array_equal(np.array([[0, 0], [4, 4], [1, 1], [2, 2], [3, 3], ]), m2.vertices)
    assert len(m2.faces) == 5
    assert is_vararray_equal(m2.faces,
                             list(np.array([[0, 1, 2], [1, 2, 0], [0, 2, 3], [2, 3, 3], [3, 4, 3]])),
                             rolling=False)
...
def test_remove_duplicate_faces():
    ...
    assert all(np.array_equal(a, b) for a, b in zip(m.faces, create_pyramid().faces))
    assert all(np.array_equal(a, b) for a, b in
               zip(m.parts, create_pyramid().parts + create_pyramid().parts))
    assert len(m.parts) == 3
    assert are_edges_equal(m.edges, create_pyramid().edges)
...
def test_remove_duplicate_parts():
    ...
    assert all(np.array_equal(a, b) for a, b in zip(m.parts, create_pyramid().parts))
    assert len(m.parts) == 1
    assert are_edges_equal(m.edges, create_pyramid().edges)


def test_split_mesh_into_objects():
    ...
    new_faces_1 = m.split_mesh_into_objects()
    assert len(new_faces_1) == 3
    ...
    assert m1 in new_faces_1
    assert m2 in new_faces_1
    assert m3 in new_faces_1
    # case no parts
    m4 = Mesh(
        vertices=np.arange(24).reshape((8, 3)),
        faces=[[0, 1, 0], [2, 3, 4], [3, 2, 4], [4, 5, 6]],
    )
    new_faces_2 = m4.split_mesh_into_objects()
    assert len(new_faces_2) == 3
    # one object with vertices 0 and 1
    assert Mesh(vertices=np.arange(6).reshape((2, 3)), faces=[[0, 1, 0]]) in new_faces_2
    # one object with vertices 2,3,4,5,6
    assert Mesh(vertices=np.arange(start=6, stop=21).reshape((5, 3)),
                faces=[[0, 1, 2], [1, 0, 2], [2, 3, 4]]) in new_faces_2
    # one object with vertex 7
    assert Mesh(vertices=np.arange(start=21, stop=24).reshape((1, 3)), faces=None) in new_faces_2

    # case no faces and parts
    m5 = Mesh(
        vertices=np.arange(24).reshape((8, 3)),
        faces=None,
    )
    new_faces3 = m5.split_mesh_into_objects()
    assert len(new_faces3) == 8
