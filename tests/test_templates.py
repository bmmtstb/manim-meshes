...
def test_basic_triangle():
    ...
    assert len(mesh.vertices) == 3  # there are 3 verts in total
    assert len(mesh.faces) == 1  # there is one face
    assert mesh.faces[0].shape[0] == 3  # face has 3 verts
...
def test_create_grid():
    ...
    assert len(m_2d.find_vertex(np.array([3, 1]))) == 1
    assert len(m_2d.vertices) == 7 * 3
    assert any(np.array_equal(np.array([0, 1, 8, 7]), face) for face in m_2d.faces)
    assert any(np.array_equal(np.array([8, 9, 16, 15]), face) for face in m_2d.faces)
    assert len(m_2d.faces) == (7 - 1) * (3 - 1)
    # 1D
